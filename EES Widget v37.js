...
// Set widget colors
const row1Color= "#F5BF10"; // mustard // controls header color
const row2Color= "#EDEEE9"; // creamy white // controls data color
const row3Color= "#C6C6BD"; // darker creamy white // controls additional data color
const eesColor = row1Color; // controls EES crown color
...
// Define file names
let fileNames = ["BMI", "GMR", "EES", "TBK", "MAIN", "STORE", "INSTA", "YT"]
...
let instaFilePath = fm.joinPath(dir, "INSTA.txt")
let instaText = fm.readString(instaFilePath)
let instaOld = instaText;
...
textOtotoyRow3.font = new Font("HelveticaNeue-LightItalic", 12);
textOtotoyRow3.textColor = new Color(row3Color);
...
textWebRow3.font = new Font ("HelveticaNeue-LightItalic", 12);
textWebRow3.textColor = new Color (row3Color);
...
textBmiStack1.centerAlignContent();
...
textBmiRow3.font = new Font("HelveticaNeue-LightItalic", 12);
textBmiRow3.textColor = new Color(row3Color);
...
// Function to extract the dateCreated value from the HTML content
function extractDateCreated(htmlContent) {
  const headlineIndex = htmlContent.indexOf('headline');
  if (headlineIndex !== -1) {
    const dateCreatedStart = htmlContent.lastIndexOf('dateCreated":"', headlineIndex);
    if (dateCreatedStart !== -1) {
      const dateCreatedEnd = htmlContent.indexOf('"', dateCreatedStart + 14);
      return htmlContent.slice(dateCreatedStart + 14, dateCreatedEnd);
    }
  }
  return null;
}
...
(async () => {
  try {
    const url = 'https://www.instagram.com/easyeyesound/';
    const htmlContent = await fetchHTML(url);
    const extractedDate = extractDateCreated(htmlContent);

    if (extractedDate) {
      // console.log(extractedDate);
    if (extractedDate != instaOld) {
    let notification = new Notification();
    notification.title = "New EES Instagram Post";
    notification.body = `There's a new Instagram post from Easy Eye Sound! Check it out!`;
    notification.openURL = 'https://www.instagram.com/easyeyesound/';
    notification.schedule();
    
    // Update the text file with the new value
  fm.writeString(instaFilePath, extractedDate.toString());
  }
    else {}
    } else {
      console.log('Date not found in HTML content.');
    }
  } catch (error) {
    console.log(error);
  }
...
// Other notifications: YouTube

const url = "https://m.youtube.com/@EasyEyeSound/videos";
...
function extractData(content) {
  const regex = /(.{120})\s+by Easy Eye Sound/g;
...
async function run() {
  try {
    const content = await fetchContent(url);
    const ytData = extractData(content);

    if (ytData) {
      // console.log(data);
    if (ytData != ytOld) {
    let notification = new Notification();
    notification.title = "New EES YouTube Upload";
    notification.body = `There's a new YouTube upload from Easy Eye Sound! Check it out!`;
    notification.openURL = 'https://m.youtube.com/@EasyEyeSound/videos';
    notification.schedule();
    
    // Update the text file with the new value
  fm.writeString(ytFilePath, ytData.toString());
    }
    else {}
    }
  } catch (error) {
