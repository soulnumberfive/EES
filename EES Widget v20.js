...
for (let i = 0; i < urls.length; i++) {
  ...
  // Step 4: Remove the day name and keep only the month and day number
  let formattedDates = dates.map(date => date.replace(/[A-Z][a-z]{2},\s/, ''));

  // Step 5: Add the formatted dates to the list of all dates
  allDates = allDates.concat(formattedDates);
}

// Step 6: Remove duplicates from the list of dates
let uniqueDates = Array.from(new Set(allDates));

// Step 7: Combine the unique dates into one string
...
const expectedDates = "Jun 15";

if (datesString !== "Jun 15, Jun 17, Jun 18, Jun 19, Jun 21, Jun 22, Jun 24, Jun 27, Jun 28, Jun 30, Jul 1, Jul 4, Jul 6, Jul 7, Jul 8, Jul 10, Aug 14, Aug 24, Aug 25, Aug 26, Aug 27, Sep 16") {
  let notification = new Notification();
