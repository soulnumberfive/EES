...
// Set widget colors
const row1Color= "#ECAB24"; // mustard yellow // controls header color
const row2Color= "#EDEEE9"; // creamy white // controls data color
const eesColor = "#ECAB24"; // mustard yellow // controls EES crown color
const footColor= "#9a9a93"; // creamy gray // controls footnote color
const tintColor= new Color("#272727", 0.75); // black // controls background image tint (w/ opacity)
...
let albumArtURL, artist, releaseTitle, formattedDate;
const regex = /\/_\/default\/p\/([^"]+)/;
const match = htmlContent.match(regex);
if (match && match[1]) {
  const extractedUrl = match[1];
  const finalUrl = `https://ototoy.jp/_/default/p/${extractedUrl}`;
...
textOtotoyRow1.font = new Font("HelveticaNeue-CondensedBold", 14);
...
textOtotoyRow2.font = new Font("HelveticaNeue-Bold", 17);
...
textOtotoyRow3.font = new Font("HelveticaNeue-Italic", 13);
textOtotoyRow3.textColor = new Color(row2Color);
textOtotoyRow3.lineLimit = 1;

// Notifications
if (releaseTitle != notifRel1) {
...
try {
  ...
  console.log(tbkRelease);
  
  if (tbkRelease != notifRel2) {
    let notification = new Notification();
...
// Get EES main website update time
  ...
  const dates = [];
  let match2;
  while ((match2 = dateRegex.exec(updateHtml)) !== null) {
  ...
  const diffInMsMain = Date.now() - lastUpdate;
  const diffInDaysMain = diffInMsMain / (1000 * 60 * 60 * 24);

console.log(diffInDaysMain)
...
let minDiffInDays = Math.min(diffInDaysMain, diffInDaysStore);

console.log(minDiffInDays);

// Create header
const textWebRow1 = widget.addText("LATEST WEBSITE UPDATE                       ");
textWebRow1.font = new Font("HelveticaNeue-CondensedBold", 14);
...
const textWebRow3 = textWebStack.addText (`Main: ${diffInDaysMain.toFixed (1)} – Store: ${diffInDaysStore. toFixed(1)}`);
...
textBmiRow1.font = new Font("HelveticaNeue-CondensedBold", 14);
...
textBmiRow2.font = new Font("HelveticaNeue-Bold", 17);
...
textBmiRow3.font = new Font("HelveticaNeue-Italic", 13);
...
const checkText = widget.addText(`Refreshed: ${currentDateTime}`);
checkText.font = new Font("HelveticaNeue-Light", 10);
checkText.textColor = new Color(footColor);
