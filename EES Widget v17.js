...
const ototoyReq = new Request(ototoyUrl);
const ototoyHtml = await ototoyReq.loadString();
const regex = /(<([^>]+)>)/ig;
const text = ototoyHtml.replace(regex, "");
const startIndex = text.indexOf("12&rarr;") + 8;
const endIndex = text.indexOf("&larr;1");
const releaseInfo = text.slice(startIndex, endIndex).trim();
const releases = releaseInfo.split("\n").filter(row => row.trim() !== "");
const firstRelease = releases[1].trim();
const secondRelease = releases[0].trim();
  
if (secondRelease != "Coal Black Mattie") {
let notification = new Notification();
notification.title = "New EES Release";
notification.body = `There's a new EES release from ${firstRelease}! It's titled ${secondRelease}!`;
notification.schedule();
  }
else {}

const url = 'https://ototoy.jp/labels/226974';
...
const html = await webView.evaluateJavaScript(`document.documentElement.innerHTML`);

// Create a temporary container to parse the HTML
const tempContainer = new WebView();
await tempContainer.loadHTML(html);
...
ototoyTitle.font = Font.boldSystemFont(18);
const ototoyText3 = ototoyStack.addText(`${firstRelease}`);
ototoyText3.font = Font.boldSystemFont(22);
ototoyText3.textColor = new Color("#ff547c");
// const ototoyText2 = ototoyStack.addText(" - ");
// ototoyText2.font = Font.italicSystemFont(18);
// const ototoyText = ototoyStack.addText(`${secondRelease}`);
// ototoyText.font = Font.boldSystemFont(20);
// ototoyText.textColor = new Color("#ff547c");
